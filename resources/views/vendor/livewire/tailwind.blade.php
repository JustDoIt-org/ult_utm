<div>
    @if ($paginator->hasPages())
        <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-between">
            ...
            <div class="hidden sm:flex-1 sm:flex sm:flex-col sm:items-center sm:gap-5 sm:justify-between sm:overflow-hidden">
                <div>
                    <p class="text-sm text-gray-700 leading-5 text-nowrap">
                ...
                <div>
                    <span class="relative z-0 inline-flex rounded-md shadow-sm mb-2">
                        <span>
                            {{-- Previous Page Link --}}
                            @if ($paginator->onFirstPage())
                                <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}" class="aria-disabled:hidden">
                                        ...
                                        @if ($page == $paginator->currentPage())
                                            @if ($paginator->onFirstPage())
                                                <span aria-current="page">
                                                    <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-semibold text-yellow-500 bg-white border border-yellow-300 rounded-s-lg transition-all scale-110 cursor-default leading-5 select-none">{{ $page }}</span>
                                                </span>
                                            @else
                                                @if($paginator->onLastPage())
                                                    <span aria-current="page">
                                                        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-semibold text-yellow-500 bg-white border border-yellow-300 rounded-e-lg transition-all scale-110 cursor-default leading-5 select-none">{{ $page }}</span>
                                                    </span>
                                                @else
                                                    <span aria-current="page">
                                                        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-semibold text-yellow-500 bg-white border border-yellow-300 transition-all scale-110 cursor-default leading-5 select-none">{{ $page }}</span>
                                                    </span>
                                                @endif
                                            @endif
                                        @else
                        ...
                        <span>
                            ...
                            @else
                                <span aria-disabled="true" aria-label="{{ __('pagination.next') }}" class="aria-disabled:hidden">
