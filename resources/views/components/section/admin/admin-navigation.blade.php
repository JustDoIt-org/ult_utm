<div>
  <x-element.sidebar-anchor route="admin.dashboard">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
      <path
        d="M11.47 3.841a.75.75 0 0 1 1.06 0l8.69 8.69a.75.75 0 1 0 1.06-1.061l-8.689-8.69a2.25 2.25 0 0 0-3.182 0l-8.69 8.69a.75.75 0 1 0 1.061 1.06l8.69-8.689Z" />
      <path
        d="m12 5.432 8.159 8.159c.03.03.06.058.091.086v6.198c0 1.035-.84 1.875-1.875 1.875H15a.75.75 0 0 1-.75-.75v-4.5a.75.75 0 0 0-.75-.75h-3a.75.75 0 0 0-.75.75V21a.75.75 0 0 1-.75.75H5.625a1.875 1.875 0 0 1-1.875-1.875v-6.198a2.29 2.29 0 0 0 .091-.086L12 5.432Z" />
    </svg>
    <span>Dashboard</span>
  </x-element.sidebar-anchor>

  <x-element.sidebar-anchor route="admin.users">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
      <path fill-rule="evenodd"
        d="M7.5 6a4.5 4.5 0 1 1 9 0 4.5 4.5 0 0 1-9 0ZM3.751 20.105a8.25 8.25 0 0 1 16.498 0 .75.75 0 0 1-.437.695A18.683 18.683 0 0 1 12 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 0 1-.437-.695Z"
        clip-rule="evenodd" />
    </svg>
    <span>Users</span>
  </x-element.sidebar-anchor>

  <x-element.sidebar-anchor route="admin.faculties">
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
        <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75M6.75 21v-3.375c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21M3 3h12m-.75 4.5H21m-3.75 3.75h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Z" />
    </svg>
    <span>Faculties</span>
  </x-element.sidebar-anchor>

  <x-element.sidebar-anchor route="admin.roles">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
      <path fill-rule="evenodd"
        d="M12.516 2.17a.75.75 0 0 0-1.032 0 11.209 11.209 0 0 1-7.877 3.08.75.75 0 0 0-.722.515A12.74 12.74 0 0 0 2.25 9.75c0 5.942 4.064 10.933 9.563 12.348a.749.749 0 0 0 .374 0c5.499-1.415 9.563-6.406 9.563-12.348 0-1.39-.223-2.73-.635-3.985a.75.75 0 0 0-.722-.516l-.143.001c-2.996 0-5.717-1.17-7.734-3.08Zm3.094 8.016a.75.75 0 1 0-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.14-.094l3.75-5.25Z"
        clip-rule="evenodd" />
    </svg>
    <span>Roles</span>
  </x-element.sidebar-anchor>

  {{-- Dropdown  Visit --}}
  <section x-data="{
      isActive: false,
      toggle() {
          this.isActive = !this.isActive
      }
  }">

    <!-- Button -->
    <button x-ref="button" @click="toggle()" type="button"
      class="flex justify-between items-center w-full p-4 text-slate-400 font-semibold hover:bg-slate-50 hover:text-slate-600">
      <div class="flex gap-5">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
          <path
            d="M12.75 12.75a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0ZM7.5 15.75a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5ZM8.25 17.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0ZM9.75 15.75a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5ZM10.5 17.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0ZM12 15.75a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5ZM12.75 17.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0ZM14.25 15.75a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5ZM15 17.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0ZM16.5 15.75a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5ZM15 12.75a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0ZM16.5 13.5a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Z" />
          <path fill-rule="evenodd"
            d="M6.75 2.25A.75.75 0 0 1 7.5 3v1.5h9V3A.75.75 0 0 1 18 3v1.5h.75a3 3 0 0 1 3 3v11.25a3 3 0 0 1-3 3H5.25a3 3 0 0 1-3-3V7.5a3 3 0 0 1 3-3H6V3a.75.75 0 0 1 .75-.75Zm13.5 9a1.5 1.5 0 0 0-1.5-1.5H5.25a1.5 1.5 0 0 0-1.5 1.5v7.5a1.5 1.5 0 0 0 1.5 1.5h13.5a1.5 1.5 0 0 0 1.5-1.5v-7.5Z"
            clip-rule="evenodd" />
        </svg>

        <span>Visit</span>
      </div>

      <!-- Heroicon: chevron-up -->
      <template x-transition x-if="isActive">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
          <path fill-rule="evenodd"
            d="M9.47 6.47a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 1 1-1.06 1.06L10 8.06l-3.72 3.72a.75.75 0 0 1-1.06-1.06l4.25-4.25Z"
            clip-rule="evenodd" />
        </svg>
      </template>

      <!-- Heroicon: chevron-down -->
      <template x-transition x-if="!isActive">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
          <path fill-rule="evenodd"
            d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
            clip-rule="evenodd" />
        </svg>
      </template>
    </button>

    {{-- Panel --}}
    <div x-transition x-show="isActive" class="scale-90 transition-all">
      <x-element.sidebar-anchor route="admin.informasi-kouta">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
          <path
            d="M5.625 3.75a2.625 2.625 0 1 0 0 5.25h12.75a2.625 2.625 0 0 0 0-5.25H5.625ZM3.75 11.25a.75.75 0 0 0 0 1.5h16.5a.75.75 0 0 0 0-1.5H3.75ZM3 15.75a.75.75 0 0 1 .75-.75h16.5a.75.75 0 0 1 0 1.5H3.75a.75.75 0 0 1-.75-.75ZM3.75 18.75a.75.75 0 0 0 0 1.5h16.5a.75.75 0 0 0 0-1.5H3.75Z" />
        </svg>
        <span>Informasi Kouta</span>
      </x-element.sidebar-anchor>
    </div>

    <div x-transition x-show="isActive" class="scale-90 transition-all">
        <x-element.sidebar-anchor route="admin.pengajuan-kunjungan">
            <x-heroicon-s-clipboard-document-list class="size-6" />
          <span>Pengajuan Kunjungan</span>
        </x-element.sidebar-anchor>
    </div>
  </section>


  {{-- End Section Visit --}}

  {{-- Dropdown PPID --}}
  <section x-data="{
      isActive: false,
      toggle() {
          this.isActive = !this.isActive
      },

      activate() {
          this.isActive = true
      }
  }">

    <!-- Button -->
    <button x-ref="button" @click="toggle()" type="button"
      class="flex justify-between items-center w-full p-4 text-slate-400 font-semibold hover:bg-slate-50 hover:text-slate-600">
      <div class="flex gap-5">
        <x-heroicon-s-wallet class="size-6" />
        <span>PPID</span>
      </div>

      <!-- Heroicon: chevron-up -->
      <template x-transition x-if="isActive">
        <x-heroicon-s-chevron-up class="w-4 h-4" />
      </template>

      <!-- Heroicon: chevron-down -->
      <template x-transition x-if="!isActive">
        <x-heroicon-s-chevron-down class="w-4 h-4" />
      </template>
    </button>

    {{-- Panel --}}
    <div x-transition x-show="isActive" class="scale-90 transition-all">
      <x-element.sidebar-anchor route="admin.request" @click="activate()">
        <x-heroicon-s-clipboard-document-list class="size-6" />
        <span>Request</span>
      </x-element.sidebar-anchor>
    </div>

    {{-- Panel --}}
    <div x-transition x-show="isActive" class="scale-90 transition-all">
      <x-element.sidebar-anchor route="admin.aspirasi-pengaduan">
        <x-heroicon-s-newspaper class="size-6" />
        <span>Pengaduan & Aspirasi</span>
      </x-element.sidebar-anchor>
    </div>

    {{-- Panel --}}
    <div x-transition x-show="isActive" class="scale-90 transition-all">
      <x-element.sidebar-anchor route="admin.survey">
        <x-heroicon-s-clipboard-document-check class="size-6" />
        <span>Survey</span>
      </x-element.sidebar-anchor>
    </div>

  </section>

  <x-element.sidebar-anchor route="admin.ult-informations">
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
      class="w-6 h-6">
      <path stroke-linecap="round" stroke-linejoin="round"
        d="M9 17.25v1.007a3 3 0 0 1-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0 1 15 18.257V17.25m6-12V15a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 15V5.25m18 0A2.25 2.25 0 0 0 18.75 3H5.25A2.25 2.25 0 0 0 3 5.25m18 0V12a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 12V5.25" />
    </svg>
    <span>ULT Informations</span>
  </x-element.sidebar-anchor>

  <x-element.sidebar-anchor route="admin.faqs">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
      <path fill-rule="evenodd"
        d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm11.378-3.917c-.89-.777-2.366-.777-3.255 0a.75.75 0 0 1-.988-1.129c1.454-1.272 3.776-1.272 5.23 0 1.513 1.324 1.513 3.518 0 4.842a3.75 3.75 0 0 1-.837.552c-.676.328-1.028.774-1.028 1.152v.75a.75.75 0 0 1-1.5 0v-.75c0-1.279 1.06-2.107 1.875-2.502.182-.088.351-.199.503-.331.83-.727.83-1.857 0-2.584ZM12 18a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Z"
        clip-rule="evenodd" />
    </svg>
    <span>FAQ</span>
  </x-element.sidebar-anchor>
</div>
