  ...
  @switch($type)
    @case('input_text')
      @if ($textfield === $kategori)
        <input type="text" class="ml-2 h-3 w-40 rounded-md p-3 focus:ring-secondary focus:border-secondary"
    ...
    @case('text_with_upload_file')
      @if ($textfield === $kategori)
        <div class="mt-3">
          <input type="text" class="ml-2 h-3 w-40 rounded-md p-3 focus:ring-secondary focus:border-secondary" required
            wire:model.live="{{ $input_text_field }}" placeholder="{{ $placeholder }}">
          @error($input_text_field)
            <div class="text-red-600">
              <strong>{{ $message }}</strong>
