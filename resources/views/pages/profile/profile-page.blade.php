  ...
  <div class="mx-auto space-y-6 max-w-7xl w-[400px] sm:w-[80%] md:w-[90%] lg:w-[80%]">
    ...
    <div class="flex flex-col md:flex-row bg-white shadow rounded-lg">
      <div class="w-full md:w-1/2 lg:w-2/4">
        <div
          class="flex justify-center items-center md:h-full font-bold text-white text-8xl rounded-t-lg md:rounded-s-lg uppercase">
          <img src="{{ Auth::user()->picture }}" alt="{{ Auth::user()->name }}"
            class="rounded-t-lg md:rounded-s-lg object-fill">
        </div>
      </div>
        ...
        <header class="flex justify-between">
          ...
          <div>
            <x-element.dropdown.container>
              ...
              <x-slot:content>
                <button
                  class="flex gap-3 items-center w-full px-3 py-1 text-slate-500 hover:text-yellow-700 hover:bg-yellow-100 hover:rounded-full"
                  @click="$dispatch('open-modal', {name: 'change-password-form-modal'})">
                  <x-heroicon-s-key width="16" />
                  Reset Password
                </button>
                <button
                  class="flex gap-3 items-center w-full px-3 py-1 text-slate-500 hover:text-yellow-700 hover:bg-yellow-100 hover:rounded-full"
                  @click="$dispatch('open-modal', {name: 'update-account-form-modal', id: {{ auth()->id() }}})">
