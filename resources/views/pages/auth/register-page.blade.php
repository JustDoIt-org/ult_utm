<div class="flex flex-col md:flex-row gap-6 md:gap-0 justify-around items-center">
  <form wire:submit="register" class="w-full">
    ...
    <p class="text-sm text-slate-600">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Esse repellendus vitae
      explicabo.</p>
    ...
    <button type="submit" wire:loading.class="opacity-50"
      class="text-white font-semibold border border-secondary bg-secondary hover:opacity-85 transition-all rounded-full py-2 w-full text-center mt-5">
      <p wire:loading wire:target="register">{{ __('Loading...') }}</p>
      <span wire:loading.remove wire:target="register"> {{ __('Sign in') }}</span>
    </button>
  </form>
  ...
  <div class="w-full flex flex-col items-center gap-3">
    <a href="/auth/redirect"
      class="flex flex-row items-center justify-center gap-5 font-semibold border border-slate-800 bg-white rounded-full py-1 w-full text-center">
      <x-element.icons.icon-google />
      {{ __('Continue with Google') }}
    </a>
    <span class="text-sm">Already have an account ? <a wire:navigate href="{{ route('login') }}"
        class="font-bold text-secondary hover:underline cursor-pointer">Sign in</a></span>
  </div>
</div>
