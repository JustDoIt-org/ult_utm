<section class="mr-10 my-14 p-4">
  <form wire:submit="save" class="w-full rounded-xl bg-white p-4">
    <h1 class="text-center text-2xl font-semibold capitalize">Form Pengajuan Kunjungan Langsung</h1>
    ...
    {{-- Kode Kunjungan --}}
    <x-element.layout.vertical name="kode_kunjungan" label="Kode Kunjungan">
      <x-element.input.line type="text" wire:model="kode_kunjungan" />
    </x-element.layout.vertical>

    <div wire:target="save" class="flex flex-col gap-2">
      <section>
        <button wire:target="save" wire:loading.attr="disabled" wire:loading.class="opacity-50" type="submit"
          class="w-full bg-secondary rounded-lg py-1 text-white font-semibold transition-all hover:scale-95">
